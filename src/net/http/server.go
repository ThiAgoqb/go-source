...
func (c *conn) readRequest(ctx context.Context) (w *response, err error) {
	...
	//当前时间
	t0 := time.Now()
	//读取header头的超时时间 如果是0 || 负数 不设超时
	if d := c.server.readHeaderTimeout(); d > 0 {
		//超时时间=当前时间+超时时间值
		hdrDeadline = t0.Add(d)
	}
	//读取header及body的超时时间 如果是0 || 负数 不设超时
	if d := c.server.ReadTimeout; d > 0 {
		//总超时时间=当前时间+超时时间值
		wholeReqDeadline = t0.Add(d)
	}
	//设置连接的header读取超时时间截止点
...
func (c *conn) setState(nc net.Conn, state ConnState, runHook bool) {
	srv := c.server
	switch state {
	//如果新建状态 当前连接存储在server.activeConn map结构中
	case StateNew:
		srv.trackConn(c, true)
	//如果是劫持、关闭状态 当前连接从server.activeConn 的map中移除
...
func (c *conn) serve(ctx context.Context) {
	...
	for {
		//创建response 将request 及requestbody封装在response中
...
func (srv *Server) Serve(l net.Listener) error {
	...
	//将listen加入到server的listenersMap中 执行完需要清除
	...
	//如果用户自定义获取上下文使用则使用用户自定义获取上下文的方法
	...
	//如果接收连接失败后需要休眠多久再次接收连接 --使用动态规划 默认5ms 每次失败 在原来基础上*2 最大休眠时间是1s
	...
	for {
		...
		if err != nil {
			...
			//断言err 是否是net.Error类型 并且这个error是临时错误
			if ne, ok := err.(net.Error); ok && ne.Temporary() {
				//配置延迟接收 如果是0 设置5ms
				//否则延迟增加一倍 tempDelay*2
				if tempDelay == 0 {
		...
		connCtx := ctx
		//用户是否自定义从连接中获取context 有自定义则使用自定义获取
